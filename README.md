# ChronoSpatial: Spatio-Temporal Daily Schedule Optimizer

> **Turning dead time between fixed commitments into mathematically optimized, planned time.**

ChronoSpatial bridges physical campus spatial topology with temporal constraint optimization. Built in standard **C99** (core data structures & algorithms) and **C++17** (object-oriented application logic) with **zero external dependencies**.
...
│       └── ChronoSpatialSystem.h # Top-level orchestrator & interactive CLI
├── src/
...
### Build & Run
```bash
# 1. Compile everything into bin/chronospatial
make all
...
3. **True Usable Time & Spatio-Temporal Venue Search**:
   ...
   $$\text{True Usable Time} = \text{GapDuration} - \text{walk}(u, v) - \text{walk}(v, w) - \text{SafetyBuffer}$$
...
## 4. Documentation
For comprehensive technical documentation, refer to the [docs/](file:///docs/) directory:
